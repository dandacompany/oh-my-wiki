...
- **Cross-session continuity** — Claude Code and Codex recall at `SessionStart`, `UserPromptSubmit`, and `PreToolUse`, then stage a small same-project snapshot at `PreCompact` and `Stop`; hook watchdogs fail open, common secret patterns are redacted, and nothing is promoted into a vault automatically
