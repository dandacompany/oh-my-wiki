...
categories:  # before → after
  A-2 '~를 통해' 남발: 4 → 1 (코드/이름 인접 제외)
  C(번역투 em-dash '—' 문장 내 삽입): 7 → 2 (의미상 동격 dash만 유지)
  I-1 '~인 것이다' 류 메타 결말: 1 → 0
self_check:
  - 고유명사·수치·날짜·코드·백틱·인용 100% 보존: ✅
  - 변경률 30% 이하 (~3%): ✅
  - 장르 이탈 없음 (기술 문서 유지): ✅
  - register 보존 (~합니다/~입니다 격식체): ✅
  - 문단 경계·개수·순서 보존 (100개 \n\n): ✅
  - 인공 표현 추가 없음: ✅
highlights:
  - id: C(em-dash)
    before: "입력하는 프롬프트 형태로 표시됩니다 — CLI 출력이 아닙니다."
    after: "입력하는 프롬프트 형태로 표시됩니다. CLI 출력이 아닙니다."
  - id: A-2
    before: "ingest 워크플로를 통해 추가됩니다."
    after: "ingest 워크플로로 추가됩니다."
  - id: C(em-dash)
    before: "별도 커맨드가 필요 없습니다 — 스킬이 입력한 내용에 따라"
    after: "별도 커맨드가 필요 없습니다. 스킬이 입력한 내용에 따라"
  - id: A-2/문맥
    before: "웹 검색을 통한 ... 오픈 웹에서 결과를 가져오는 것으로"
    after: "웹 검색을 ... 수행합니다. 오픈 웹에서 결과를 가져오며"
residual_findings: |
  의도 유지 — autoresearch 문단의 동격 삽입 dash(— 라운드별 주장, 출처, 공백 —)와
  persona 정의 문단의 '용어 — 설명' dash는 기술 문서에서 정의/동격 구문으로 기능하므로
  보존. '~할 수 있다'(A-10)는 기능 가능성 서술로 기술 문서에서 적절, 미변경.
grade_reason: "A — S1 잔존 0건, 변경률 ~3%로 의미·구조 완전 보존, 100개 문단 경계 유지. 번역투 em-dash와 '~를 통해' 남발만 선별 제거."
-->
